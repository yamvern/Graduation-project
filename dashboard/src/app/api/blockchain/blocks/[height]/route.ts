...
export async function GET(_: Request, context: { params: Promise<{ height: string }> }) {
  const { height } = await context.params;
  const token = await getBearerTokenFromCookies();
  if (!token) return NextResponse.json({ message: "Unauthorized" }, { status: 401 });
  try {
    const res = await fetch(`${getBackendBaseUrl()}/api/v1/blockchain/blocks/${height}`, {
